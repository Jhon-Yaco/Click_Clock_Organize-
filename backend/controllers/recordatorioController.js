...
// ✅ Eliminación de un solo recordatorio por ID
exports.eliminarRecordatorio = (req, res) => {
    const recordatorioId = req.params.id; // ✅ Recibiendo el ID desde la URL

    if (!recordatorioId) {
        return res.status(400).json({ mensaje: "❌ ID no proporcionado" });
    }

    console.log(`🔍 Intentando eliminar recordatorio con ID: ${recordatorioId}`); // Depuración en consola del backend
