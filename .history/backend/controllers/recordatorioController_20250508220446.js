...
exports.eliminarRecordatorio = (req, res) => {
    ...
    const { titulo, fecha } = req.body; // ✅ Permitir eliminación con título y fecha

    let sql, params;
    
    if (recordatorioId) {
        sql = `DELETE FROM recordatorios WHERE id = ?`;
        params = [recordatorioId];
    } else if (titulo && fecha) {
        sql = `DELETE FROM recordatorios WHERE titulo = ? AND fecha = ?`;
        params = [titulo, fecha];
    } else {
        return res.status(400).json({ mensaje: "❌ Debes proporcionar un ID o título y fecha." });
    }
    ...
    db.query(sql, params, (err, result) => {
        if (err) {
            ...
            console.log('🗑️ Recordatorio eliminado correctamente:', result);
