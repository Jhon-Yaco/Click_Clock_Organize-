...
exports.eliminarRecordatorio = (req, res) => {
    const { titulo, fecha } = req.body; // ✅ Recibir título y fecha en lugar de ID
    ...
    db.query(sql, [titulo, fecha], (err, result) => {
        if (err) {
        ...
        console.log(`🗑️ Recordatorio eliminado: ${titulo} - ${fecha}`);
