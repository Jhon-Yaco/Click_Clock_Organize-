document.addEventListener('DOMContentLoaded', function () {
    ...
    // ✅ Función para eliminar un recordatorio específico (ahora lo borra de MySQL)
    function eliminarRecordatorio(recordatorio) {
        console.log(`🗑️ Eliminando recordatorio:`, recordatorio);

        fetch(`${API_URL}/${recordatorio.id}`, { // ✅ Se usa el ID del recordatorio
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' }
        })
        .then(res => res.json())
        .then(() => {
            recordatorios = recordatorios.filter(r => r.id !== recordatorio.id); // ✅ Lo elimina de la lista local
            renderizarListaRecordatorios(); // ✅ Refresca la interfaz
        })
        .catch(error => console.error('❌ Error al eliminar:', error));
    }

    // ✅ Función para eliminar todos los recordatorios (NO SE TOCA)
    ...
    setInterval(() => {
        ...
        recordatorios.forEach(rec => {
            ...
            if (diferencia <= 3000 && !rec.mostrado) { // dentro de los 3 segundos
                reproducirSonido(); // ✅ Reproducir sonido sin necesidad de aceptar la alerta
