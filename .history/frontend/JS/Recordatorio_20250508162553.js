document.addEventListener('DOMContentLoaded', function () {
    ...
    function eliminarRecordatorio(recordatorioId) {
        fetch(`${API_URL}/${recordatorioId}`, { // ✅ Se envía el ID correcto al backend
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' }
        })
        .then(res => res.json())
        .then(data => {
            console.log(`🗑️ Recordatorio con ID ${recordatorioId} eliminado del backend:`, data);
            cargarRecordatoriosDesdeBackend(() => renderizarListaRecordatorios()); // ✅ Refrescar lista desde el backend
        })
